...
<div id="chat-container">
    <div class="user-list">
        ...
        <div class="tabcontent" id="Architects" style="display:none;">
            <ul id="list">
            @if($projects->isEmpty())
            <li>No assigned Architects</li>
            @else
                @foreach($projects as $project)
                    @if ($project->staff->user->role === 'architect')
                        <li class="user" data-name="{{ $project->staff->user->name }}"
                            data-image="{{ asset($project->staff->profile_image) }}"
                            onclick="openChat('{{ $project->staff->user->id }}', '{{ $project->staff->user->name }}')">
...
<script>
    ...
    function fetchMessages(receiverId, fromId) {
        ...
        $.ajax({
            url: `/client/fetch-messages/${receiverId}`,
            type: 'GET',
            data: { from_id: fromId },
            success: function (response) {
